...
class HabitDal {
    addHabit = async (values) => {
        let sql = `
            INSERT INTO habits (user_id, habit_name, habit_icon) 
            VALUES ($1, $2, $3) 
            RETURNING *
        `;
    ...
    addOneTimeHabit = async (values) => {
        let sql = `
            INSERT INTO one_time_habits (user_id, oth_name, oth_date) 
            VALUES ($1, $2, $3) 
            RETURNING *
        `;
        return await executeQuery(sql, values);
    ...
    getOneTimeHabits = async (userId, date = null) => {
        let sql = `
            SELECT 
                ...
                user_id, 
                oth_name AS name, 
                TO_CHAR(oth_date, 'YYYY-MM-DD') AS date, 
                oth_is_completed AS is_completed 
            FROM one_time_habits 
            WHERE user_id = $1
        `;
        let params = [userId];
        if (date) {
    ...
    toggleOneTimeHabit = async (otId, userId) => {
        let sql = `
            UPDATE one_time_habits 
            SET oth_is_completed = NOT oth_is_completed 
            WHERE oth_id = $1 AND user_id = $2
        `;
