...
class ProgressDal {
    toggleProgress = async (values) => {
        ...
        const checkSql = `
            SELECT progress_is_completed 
            FROM progress 
            WHERE user_id = $1 AND habit_id = $2 AND progress_date = $3
        `;
        ...
        if (exists.length > 0) {
            ...
            const insertSql = `
                INSERT INTO progress (user_id, habit_id, progress_date, progress_is_completed) 
                VALUES ($1, $2, $3, true)
            `;
    ...
    getProgressByDateRange = async (userId, startDate, endDate) => {
        const sql = `
            SELECT 
                ...
                user_id, 
                habit_id, 
                TO_CHAR(progress_date, 'YYYY-MM-DD') AS date, 
                progress_is_completed AS is_completed 
            FROM progress 
            WHERE user_id = $1 AND progress_date BETWEEN $2 AND $3
        `;
